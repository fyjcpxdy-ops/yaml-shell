...
# ========== 替换原harbor_address_all逻辑为以下内容 ==========
# 1. 清理原始harbor_address的所有特殊字符（引号、空格、换行、制表符）

# ========== 替换原harbor_address_all逻辑为以下内容 ==========
# 1. 清理原始harbor_address的所有特殊字符（引号、空格、换行、制表符）
harbor_address_clean=$(echo "$harbor_address" | sed -e 's/["'\''\t\n ]//g' -e '/^$/d')

# 2. 提取纯域名（移除端口，无论是否带端口）
harbor_domain_only=$(echo "$harbor_address_clean" | sed -E 's/:[0-9]+$//')

# 3. 按需添加端口（仅端口非443时）
harbor_address_all="$harbor_domain_only"
if [[ "$harbor_https_port" != "443" && -n "$harbor_https_port" ]]; then
    harbor_address_all="${harbor_domain_only}:${harbor_https_port}"
fi

# 4. 终极去重（转义特殊字符，覆盖所有重复场景）
# 转义harbor_address_all中的特殊字符（:/.），避免sed正则失效
harbor_escaped=$(echo "$harbor_address_all" | sed -e 's/\./\\./g' -e 's/:/\\:/g' -e 's/\//\\\//g')
# 移除路径中所有重复的Harbor地址前缀
harbor_address_all=$(echo "$harbor_address_all" | sed -E "s/^(${harbor_escaped})\/${harbor_escaped}/${harbor_escaped}/g" | sed -E "s/(${harbor_escaped})\/${harbor_escaped}/${harbor_escaped}/g")

# 5. 最终清理（确保无空格/引号）
harbor_address_all=$(echo "$harbor_address_all" | sed -e 's/["'\''\t\n ]//g')

# 6. 强化调试日志（必加，定位问题）
echo "===== Harbor地址最终结果 ====="
echo "原始harbor_address: [$harbor_address]"
echo "清理后harbor_address_clean: [$harbor_address_clean]"
echo "纯域名harbor_domain_only: [$harbor_domain_only]"
echo "最终harbor_address_all: [$harbor_address_all]"
echo "=============================="
...
while [[ 1 == 1 ]]; do
    ...
    if [[ $iu == "Y" ]] || [[ $iu == "y" ]]; then
		...
		shellStr="$bashpath/../deploy-middleware-operator/install.sh $harbor_address_all >> /var/log/deploy.log"
                execShell $echoStr $shellStr $num
            fi
        ...
        if [[ $num == "99" ]] || [[ $num == "7" ]]; then
            echoStr="$num 部署多网卡"
            shellStr="$bashpath/../deploy-multus/install.sh $harbor_address_all  >> /var/log/deploy.log"
            execShell $echoStr $shellStr $num
        fi
        ...
        if [[ $num == "99" ]] || [[ $num =~ ^8 ]]; then
            if [[ $env == "top-ha" ]] || [[ $env == "top" ]] || [[ $env == "pac" ]]; then
                ...
                if [[ $num == "99" ]] || [[ $num == "8" ]] || [[ $num == "8.9" ]]; then
                    echoStr="$num 部署istio"
                    shellStr="$bashpath/../deploy-istio/install.sh $harbor_address_all >> /var/log/deploy.log"
                    execShell $echoStr $shellStr $num
                fi
